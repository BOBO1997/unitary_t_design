...
def LRC(n: int, D: int, to_gate: bool = False) -> QuantumCircuit:
    """
    ...
    for l in range(1, D+1):
        if n & 1 and l & 1:
            for i in range((n - 1) // 2):
                qc.append(qi.random_unitary(4).to_instruction(), [2 * i, 2 * i + 1])
        if n & 1 and not l & 1:
            for i in range((n - 1) // 2):
                qc.append(qi.random_unitary(4).to_instruction(), [2 * i + 1, 2 * i + 2])
        if not n & 1 and l & 1:
            for i in range(n // 2):
                qc.append(qi.random_unitary(4).to_instruction(), [2 * i, 2 * i + 1])
...
def RDC(n: int, D: int, to_gate: bool = False) -> QuantumCircuit:
    """
